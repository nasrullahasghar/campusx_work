...
def predict(user_input: InputSchema):
    prediction = make_prediction(user_input.model_dump()) # it will convert json to dictionary
...
def batch_predict(user_inputs: List[InputSchema]):
    predictions = make_batch_predictions([i.model_dump() for i in user_inputs])
    return [OutputSchema(predicted_price = round(float(prediction),2)) for prediction in predictions]